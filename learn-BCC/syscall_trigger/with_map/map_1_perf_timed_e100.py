...
def print_event(cpu, data, size):
    ...
    if call_count % 100 == 0:
        ...
        print(f"CPU [{cpu}] UID {uid} comm: {event.comm.decode('utf8', 'replace')}, count: {count}")
...
print(f"[SUMMARY] Events in map: {sum(v.value for v in b['count_hash'].values())}", flush=True)
